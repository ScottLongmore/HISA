#!/usr/bin/python
"""
HISA.py - Master control driver for HISA algorithms 

example:
HISA.py -c <config.json> -l <input list file> -t <time:YYYYMMDDHHMMSS>
"""
...
LOG = logging.getLogger("HISA") #create the logger for this file
setup_logging.setup_logging("HISA","HISA.LOG")
LOG.info("Starting: {}".format(__file__)) 


# Read command line arguments 
options = argparse.ArgumentParser()
options.add_argument("-c", "--config", dest="config", help="HISA Configuration File")
options.add_argument("-i", "--input", dest="input", help="Input Specification File")
try:
    args = options.parse_args()
except:
    msg='Syntax: python HISA.py -c <config.json> -i <input spec file>'
    utils.error(LOG,msg,error_codes.EX_USAGE)

# Read master HISA JSON configuration file
try:
...
config['datetimes']=hisa.datetimes(config['inputs']["job_coverage_start"],config['inputs']["job_coverage_end"])
...
for dataname in config['datasets']: 
    ...
    try:
        ...
        for datafile in datafiles:
            properties={}
            properties['regexp']=config['datasets'][dataname]['regexp']
            properties['filepath']=utils.moveFile(workDir,datafile,dataDir) # Move data files to dataDir
            metadata[dataname].append(getattr(dataset['dataModule'],dataset['dataMethod'])(*[properties]))
    except:
...
for dataname in config['datalinks']: 
    ...
    config['datalinks'][dataname]['links']=hisa.datalinks(metadata,datasetKeys)
...
if os.path.isfile(gfsPackFile):
   ...
   if not hisa.grib2pack(gfsDate,gfsHour,gfsFhour,gfsGribFile,config['inputs']['WGRIB'],config['programs']['bin2pack']['exe'],logDir):
...
for adeck in adecks:
    ...
    stormVars=hisa.getInputAnalysisVars(afdeckInputFile)
	...
	LOG.info("Querying satellite/instrument/dataset pool: {}/{}/{} ".format(satname,instr,pool['dataset']))
        ...
        if not hisa.extractTarFiles(config['inputs']['TAR'],tarFile,"{}_{}_{}".format(stormId,satId,pool['dataset']),logDir):
	    ...
	    LOG.info("Determining files from linked dataset: {}/{}/{} ".format(satname,instr,poolname))
            ...
            poolFiles[poolname]=hisa.getLinkFilenames(links, inputDataset, inputDatasetKey, inputFilePaths, poolname)
	    ...
	    LOG.info("Querying satellite/instrument/dataset linked pool: {}/{}/{} ".format(satname,instr,poolname))
            ...
            # Extract files from pool tar file
            if not hisa.extractTarFiles(config['inputs']['TAR'],tarFile,"{}_{}_{}".format(stormId,satId,poolname),logDir):
                LOG.warning("Problem extracting files from tar file: {}, skipping to next satellite".format(tarFile))
                skipFlag=True
                break 
        ...
        for varname in pool['packed_variables']: 
            ...
            status=hisa.scaleOffsetThreshPoolTextFile(var['scale'],var['offset'],var['lowThresh'],var['highThresh'],var['inputMissing'],var['outputMissing'],packedVarFilename,filename)
            if status==False: 
        ...
        hisa.createTimeFieldFiles(satcenter['timeFile'])
	...
	# Create COORTIMES file from, COORDINATES, TIMES, satellite and instrument identifiers
        status=hisa.createCOORTIMES(afdeck['outputs']['coordFile'], satcenter['outputs']['timesFile'], satId, instr.upper(), oparet['inputs']['COORTIMES'])
